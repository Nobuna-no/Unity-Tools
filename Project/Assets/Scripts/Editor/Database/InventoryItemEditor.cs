...
public class InventoryItemEditor : EditorWindow
{
    ...
    [MenuItem("Window/Inventory Item Editor %#1")]
    static void Init()
    {
    ...
    void CreateNewItemList()
    {
        ...
        viewIndex = 1;
        inventoryItemList = CreateInventoryList.Create();
    ...
    void AddItem()
    {
        string itemName = inventoryItemList.name + "_item_" + inventoryItemList.itemList.Count.ToString();
        InventoryItem newItem = InventoryItem_AssetSupervisor.Create(itemName);
    ...
    {
        if (!InventoryItem_AssetSupervisor.Delete(inventoryItemList.itemList[index]))
            Debug.LogError("Fail to delete InventoryItem.");
